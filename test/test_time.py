...
def analytical_set(df, S_init = 1000., C_old = 50., dt=1):
    ...
    def analytical(total_t,dt, ET, Q, S,J,C_J,C_old):
        ...
        for t in range(total_t):
            ...
            for T in range(1,t+1):
                # calculate SAS
                pq[T] = S[t-T]/S[t]*expo(delta[t-T-1:t+1],phi[t-T-1:t+1])\
                    *(np.exp(delta[t]*phi[t])-1)*(np.exp((delta[t-T]+eta[t-T])*phi[t-T])-1.)/delta[t]               
                    # get C_Q
...
#%%
    ...
    data_df["S0"] = Storage_init + (data_df['J'] - data_df['Q'] - data_df['ET']).cumsum()*dt # This should be the total storage volume, solve the mass balance equation
    ...
    if makefigure:
        ...
        ax.plot(data_df[f'unsteady uniform benchark C'], alpha=0.3, lw=2, label="benchmark")
