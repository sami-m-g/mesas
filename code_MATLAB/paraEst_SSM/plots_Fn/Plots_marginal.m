function [sMean,sStd,theta]= Plots_marginal(filename,figname,path,Obsdatafile)
...
%% ------------------------------------
%% plot the figures
fig_theta = strcat(path,'fig_theta',figname);
fig_state = strcat(path,'fig_state',figname);
fig_traj  = strcat(path,'fig_trajEns',figname);
fig_jump  = strcat(path,'fig_jump',figname);
fig_thetaScatter = strcat(path,'fig_thetaScatter',figname);
...
figure(figNum); plotdensity(samples, thetaTrue, titl,prior); 
...
tN1 = length(Utrue(1,:)); t_ind = 1:max(1,floor(tN1/100)):tN1-1;
ensemble = reshape(Usample(1,:,:), tN1,[]);
plot(ensemble(t_ind,:),'cyan'); hold on;
h1 = plot(sMean(1,t_ind),'b-d','linewidth',1); hold on;
h2 = plot(Utrue(1,t_ind),'k-*','linewidth',1);
h3 = plot(sMean(1,t_ind)+sStd(1,t_ind),'m-.','linewidth',1); hold on;
h4 = plot(sMean(1,t_ind)-sStd(1,t_ind),'m-.','linewidth',1); hold on;
h5 = plot(obs(t_ind,1),'ro','linewidth',0.5); hold on;
legend([h1,h2,h5],{'MeanEst','true','Observation'},'FontSize',16);
titl = ['Ensemble trajectories of Node 3. ', 'relerr',num2str(errRel)]; 
title( titl); 
