...
sas_specs = {"Q1": {"ST": [S_m, S_0]}}

sas_specs = {"Q1": {"ST": [0, "S_0"]}}
...
sas_specs = {
    "Outflux 1": {
        "SAS fun 1": {
            "scipy.stats": gamma,
            "args": {"a": 0.62, "scale": 5724.0, "loc": 0.0},
            "nsegment": 50,
        }
    },
    "Outflux 2": {
        "SAS fun 2": {
            "scipy.stats": beta,
            "args": {"a": 2.31, "b": 0.627, "scale": 1402, "loc": 248},
            "nsegment": 50,
        }
    },
}

solute_parameters = {"C1 [mg/L]": {"C_old": 11.2}, "C2 [stone/bushel]": {"C_old": 11.2}}
...
def steady_run_continuous(
    ...
    data_df["steady_run_continuous_old_scipy"] = np.ones(timeseries_length) / 3.0
    data_df["steady_run_continuous_new_scipy"] = np.ones(timeseries_length) / 3.0
    data_df["steady_run_continuous_builtin"] = np.ones(timeseries_length) / 3.0
...
def disable_steady_run_multiple(
    timeseries_length,
    dt,
    Q_0,
    S_0,
    ...
    sas_spec = {
        "Q1": {"Fixed": {"ST": ST1}},
        "Q2": {"c21": {"ST": ST21}, "c22": {"ST": ST22}},
    }
    solute_parameters = {
        "Ca": {"C_old": C_old, "observations": ["Q1", "Q2"]},
...
def test_steady_uniform():
    ...
    pQdisc[0] = (Q_0 * (-1 + Delta + n[0] * Delta + Eta[0] * Kappa)) / (
        S_0 * Delta ** 2
    )
    ...
    model = steady_run(
        timeseries_length, dt, Q_0, S_0, C_J, n_segment=1, max_age=max_age
    )
    rdf = model.result
    ...
    if jacobian:
        ...
        dsTdSjdisc = -(
            (Q_0 * Eta * (-1 + n * Delta * (-1 + Kappa) + Kappa + Delta * Kappa))
            / (S_0 * Delta)
        )
        dmTdSjdisc = -(
            (C_J * Q_0 * Eta * (-1 + n * Delta * (-1 + Kappa) + Kappa + Delta * Kappa))
            / (S_0 * Delta)
        )
        dCQdSjdisc = (
            (C_J - C_old)
            * Eta
            * (-1 + n * Delta * (-1 + Kappa) + Kappa + Delta * Kappa)
        ) / (S_0 * Delta)
        ...
        j = 1
        dSj = (
            SAS_lookup2[timeseries_length - 1, j] - SAS_lookup[timeseries_length - 1, j]
        )
        ...
        def printcheck(rdfi, rdfp, varstr, ostr, analy):
            ...
            errcheck = (analy[:max_age, ...] - check[:max_age, ...]) / analy[
                :max_age, ...
            ]
            try:
...
#%%
    ...
    data_df = pd.DataFrame()
    data_df["old_scipy"] = np.ones(timeseries_length) * Q_0
    data_df["new_scipy"] = np.ones(timeseries_length) * Q_0
    data_df["builtin"] = np.ones(timeseries_length) * Q_0
    data_df["J"] = np.ones(timeseries_length) * Q_0 * 3.0
    data_df["Ca"] = np.ones(timeseries_length) * C_J
    data_df["S_0"] = np.ones(timeseries_length) * S_0
    data_df["S_m"] = np.ones(timeseries_length) * S_m
    data_df["old_scipy"] = np.ones(timeseries_length) / 3.0
    data_df["new_scipy"] = np.ones(timeseries_length) / 3.0
    data_df["builtin"] = np.ones(timeseries_length) / 3.0
    sas_specs = {
        "old_scipy": {
            "old_scipy_def": {
                "scipy.stats": gamma,
                "args": {"a": a, "scale": "S_0", "loc": "S_m"},
                "nsegment": n_segment,
            }
        },
        "new_scipy": {
    ...
    model.run()
    rdf = model.result
    std = rdf['C_Q'].std(axis=1)
    assert np.all(std[:29]<2) and np.all(std[29:]>2)
    # %%
    #fig = plt.gcf()
    #ax = plt.subplot(1, 1, 1)
    #ax.plot(rdf["C_Q"][:, 0, 0], '--', label="Old Scipy")
    #ax.plot(rdf["C_Q"][:, 1, 0], ':', label="New Scipy")
    #ax.plot(rdf["C_Q"][:, 2, 0], '-.', label="New Builtin")
    #plt.show(fig)
    # %%
...
def test_steady_piston_uniform():
    ...
    sTdisc = (
        ...
        + (
            ...
            * Kappa ** (m / (-1 + m * Delta))
            * (
                -((-1 + Delta + n * Delta) * Kappa ** (m / (1 - m * Delta)))
                + (-1 + m * Delta) * Kappa ** ((1 + n) / (1 - m * Delta))
                + (
                    (-1 + n * Delta) * Kappa ** (m / (1 - m * Delta))
                    + (1 - m * Delta) * Kappa ** (n / (1 - m * Delta))
                )
                * HeavisideTheta(-m + n)
            )
            * HeavisideTheta(1 - m + n)
        )
        / Delta
    )
    dsTdSjdisc = (
        ...
        * Kappa ** ((1 + n) / (1 - m * Delta))
        * (
            (1 + (1 - 2 * m + n) * Delta) * Kappa ** (m / (-1 + m * Delta))
            + (-1 + m * Delta) * Kappa ** ((1 + n) / (-1 + m * Delta))
            + Kappa ** (1 / (-1 + m * Delta))
            * (
        ...
        (
            ...
            * (
                (
                    ...
                    + Delta
                    - n * Delta
                    + (-1 + m * Delta) * Kappa ** ((1 + m - n) / (-1 + m * Delta))
                )
                * HeavisideTheta(((-1 - m + n) * S_0 * Delta) / Q_0)
                + 2
                * (
                    -1
                    + n * Delta
                    + (1 - m * Delta) * Kappa ** ((m - n) / (-1 + m * Delta))
                )
                * HeavisideTheta(((-m + n) * S_0 * Delta) / Q_0)
                + (
                    1
                    - (1 + n) * Delta
                    + (-1 + m * Delta) * Kappa ** ((1 - m + n) / (1 - m * Delta))
    ...
    pQdisc[0] = (
        ...
        * (
            ...
            + Delta
            + n[0] * Delta
            + (1 - m * Delta) * Kappa ** ((1 - m + n[0]) / (1 - m * Delta))
        )
        * HeavisideTheta(((1 - m + n[0]) * S_0 * Delta) / Q_0)
    ) / (S_0 * Delta ** 2)
    mQdisc = pQdisc * Q_0 * C_J
    dpQdSjdisc = -(
        (
            Q_0
            * Kappa ** ((1 + m - n) / (-1 + m * Delta))
            * (
                (
                    1
                    + (-1 - 2 * m + n) * Delta
                ...
                * HeavisideTheta(((-1 - m + n) * S_0 * Delta) / Q_0)
                + 2
                * (
                    (-1 + 2 * m * Delta - n * Delta) * Kappa ** (1 / (1 - m * Delta))
                    + (1 - m * Delta) * Kappa ** ((1 + m - n) / (1 - m * Delta))
                )
                * HeavisideTheta(((-m + n) * S_0 * Delta) / Q_0)
                - (
                    (1 - m * Delta) * Kappa ** ((1 + m - n) / (1 - m * Delta))
                    + (-1 + (-1 + 2 * m - n) * Delta) / Kappa ** (2 / (-1 + m * Delta))
                )
                * HeavisideTheta(((1 - m + n) * S_0 * Delta) / Q_0)
            )
        )
    ...
    dCQdSjdisc = (
        ...
        * (
            1
            - m * Delta
            + (-1 + (-1 + 2 * m - n) * Delta) * Kappa ** ((1 - m + n) / (1 - m * Delta))
            + (
                -1
                + m * Delta
                + (1 - 2 * m * Delta + n * Delta)
    ...
    dsTdSmdisc = -(
        (
            ...
            * (
                ...
                - m
                + n
                + (m - n) * Kappa ** (1 / (-1 + m * Delta)) * HeavisideTheta(-m + n)
            )
            * HeavisideTheta(1 - m + n)
        )
        / (S_0 * (-1 + m * Delta))
    ...
    dpQdSmdisc = -(
        (
            ...
            * (
                ...
                * Kappa ** (2 / (-1 + m * Delta))
                * HeavisideTheta(-1 - m + n)
                - 2 * (m - n) * Kappa ** (1 / (-1 + m * Delta)) * HeavisideTheta(-m + n)
                + (-1 + m - n) * HeavisideTheta(1 - m + n)
            )
        )
    ...
    def printcheck(rdfi, varstr, analy):
        err = (analy[:max_age, ...] - rdfi[varstr][:max_age, ...]) / analy[
            :max_age, ...
        ]
        try:
    ...
    if jacobian:
        ...
        model0 = steady_run(
            timeseries_length, dt, Q_0, S_0, C_J, j=1, ST_min=S_m, n_segment=1
        )
        rdf0 = model0.result
        SAS_lookup, _, _, _, _, _, _, _ = model._create_sas_lookup()
        ...
        j = 1
        dSj = (
            SAS_lookup0[timeseries_length - 1, j] - SAS_lookup[timeseries_length - 1, j]
        )
        ...
        modelm = steady_run(
            timeseries_length, dt, Q_0, S_0, C_J, j=0, ST_min=S_m, n_segment=1
        )
        rdfm = modelm.result
        SAS_lookup, _, _, _, _, _, _, _ = model._create_sas_lookup()
        ...
        j = 0
        dSj = (
            SAS_lookupm[timeseries_length - 1, j] - SAS_lookup[timeseries_length - 1, j]
        )
    ...
    if jacobian:
        ...
        for iq, ic, ip0 in [
            ...
            for j in range(n_segment + 1):
                ...
                dSj = (
                    SAS_lookupp[timeseries_length - 1, ip]
                    - SAS_lookup[timeseries_length - 1, ip]
                )
