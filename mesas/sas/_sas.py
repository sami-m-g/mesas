...
def make_lookup(SAS_fun, N, P_list):
    numflux = len(SAS_fun)
    SAS_lookup = np.zeros((len(P_list), N, numflux))
    for i in range(N):
        for q in range(numflux):
            SAS_lookup[:, i, q] = SAS_fun[q].invcdf_i(P_list, i)
            SAS_lookup[0, i, q] = SAS_fun[q].ST_min[i]
    return SAS_lookup


def solve(J, Q, SAS_fun, mode='RK4', ST_init=None, dt=1, n_substeps=1, P_list=None,
          full_outputs=True, CS_init=None, C_J=None, alpha=None, k1=None, C_eq=None,  C_old=None, verbose=False, debug=False):
    """Solve the SAS model for given fluxes
    ...
    Args:
        ...
        SAS_fun : SASFunctionClass or list of SASFunctionClass generated by rsas.create_function
            The number of SASFunctionClass in this list must be the same as the
    ...
    Kwargs:
        ...
        P_list : p x 1 float64 ndarray
            ...
            Used to construct a lookup table for the SAS function. Default is 101 uniformly spaced values
    ...
    nP_list = len(P_list)
    if type(SAS_fun) is np.ndarray:
        if ((SAS_fun.shape[0] == nP_list)
                and (SAS_fun.shape[1] == timeseries_length)
                and (SAS_fun.shape[2] == numflux)):
            _verbose('...assuming SAS_fun is already lookup table...')
            SAS_lookup = SAS_fun
        else:
            raise TypeError('SAS_lookup is wrong shape')
    else:
        if type(SAS_fun) is not list:
            SAS_fun = [SAS_fun]
        if numflux != len(SAS_fun):
            raise TypeError(
                'Each SAS function must have a corresponding outflow in Q. Numbers don''t match')
        for fun in SAS_fun:
            ...
            if not ('cdf_all' in fun_methods and 'cdf_i' in fun_methods):
                raise TypeError(
                    'Each SAS function must have methods SAS_fun.cdf_all and SAS_fun.cdf_i')
        _verbose('...making rsas lookup table SAS_lookup...')
        SAS_lookup = make_lookup(SAS_fun, timeseries_length, P_list)
        _verbose('...done...')
    ...
    if full_outputs:
        if numsol > 0:
            result.update({'ST': ST, 'PQ': PQ, 'WaterBalance': WaterBalance, 'MS': MS, 'MQ': MQ, 'MR': MR,
                           'C_Q': C_Q, 'SoluteBalance': SoluteBalance, 'P_list': P_list, 'SAS_lookup': SAS_lookup})
        else:
            result.update({'ST': ST, 'PQ': PQ, 'WaterBalance': WaterBalance,
                           'P_list': P_list, 'SAS_lookup': SAS_lookup})
    return result
