    ...
    def cdf_i(self, ST, i):
        return np.where(ST > self.ST_min[i],
                        np.where(ST < self.ST_max[i],
                                 1 - (1 - ((ST - self.ST_min[i]) /
                                           (self.scale[i])))**(1./(2-self.bT[i])),
                                 1.),
                        0.)
...
class _stats_SAS(SASFunctionClass):
    def __init__(self, SAS_type, params):
        ...
        if params.shape[1] > 2:
            self.shape = params[:, 2:]
            self.dist = [self.dist_class(*self.shape[i], loc=self.loc[i],
                                         scale=self.scale[i]) for i in range(N)]
        else:
...
class _exponential_SAS(SASFunctionClass):
    def __init__(self, params):
        ...
        self.rescale = np.where(np.isfinite(self.ST_max), 1 /
                                (1-np.exp(-self.lam * (self.ST_max-self.ST_min))), 1.)
...
class _gamma_SAS(SASFunctionClass):
    def __init__(self, params):
        ...
        self.rescale = np.where(np.isfinite(self.ST_max), 1 /
                                (gammainc(self.a, self.lam*(self.ST_max-self.ST_min))), 1.)
