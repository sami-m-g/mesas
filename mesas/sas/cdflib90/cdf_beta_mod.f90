MODULE cdf_beta_mod
  ...
  ! ..
  ! .. Public Statements ..
        PUBLIC :: ccum_beta, cdf_beta, cum_beta, inv_beta, cum_beta_pure
  ! ..
      CONTAINS
  ...
  !*********************************************************************
        PURE FUNCTION cum_beta_pure(x,a,b)
          USE biomath_mathlib_mod
  ! .. Function Return Value ..
          REAL (dpkind) :: cum_beta_pure
  ! ..
  ! .. Scalar Arguments ..
          REAL (dpkind), INTENT (IN) :: a, b
          REAL (dpkind), INTENT (IN) :: x
          REAL (dpkind) :: cum, ccum, xc
          INTEGER :: ierr
  ! ..
            xc = 1-x
            CALL bratio(a,b,x,xc,cum,ccum,ierr)

            cum_beta_pure = cum
  
          RETURN
  
        END FUNCTION cum_beta_pure
